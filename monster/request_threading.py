...
class RequestInfoThread(Thread): 
    ...
    def _request_info(self): 
        ...
        json_dct = {'search_title': self.job_title, \
                ...
                'search_date': current_date, 'job_site': 'monster', 'job_title': ""}
        
        json_dct['job_title'] = self.job_result.select(".title")[0].text

        try: 
            posting_company = self.job_result.select(".company")[0].text
        except: 
            posting_company = '' 
        try: 
            job_location = self.job_result.select(".location")[0].text
        except: 
            job_location = ''
        # try: 
    ...
    def _query_href(self, href): 
        ...
        #print ("Here")
        print (href)
        html = get('http://www.job-openings.monster.com' + href) if href.startswith('/') \
                    else get(href)
            #print ("Hola")        
        soup = BeautifulSoup(html.content, 'html.parser')
        if(len(soup.findAll(class_='details-content '))>0):
            texts = soup.findAll(class_='details-content ')[0].text
            i +=1
            print ("Ok"+ str(i))
        else:
            print ("error")
