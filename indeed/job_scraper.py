...
def multiprocess_pages(base_URL, job_title, job_location, page_start): 
    ...
    store_in_mongo(mongo_update_lst, 'job_postings', 'indeed_updated_final')
...
if __name__ == '__main__':
    ...
    #print (html)
    #f = open("HTML Code", "w", encoding = "utf-8")
    #f.write(html.prettify())
    try: 
        num_jobs_txt = str(html.find_all(id = 'searchCount')[0].text)
        xy = num_jobs_txt.split(" ")[-2]
        print (xy)
        cd = int(xy[0])*1000 + int(xy[2])*100 +int(xy[3])*10 +int(xy[4])  #Edit this according to query
        print (cd)
